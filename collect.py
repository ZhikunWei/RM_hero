...
#            x,y,w,h = 2*x,2*y,2*w,2*h
            if ok:
                ...
                print("Tracking x,y: {}".format([x+w/2, y+h/2]))
            else:
                armor_box = None
        ...
        uart.sendTarget(x, y, z)
        angles.append(uart.angle)
        boxes.append(armor_box)
        print(armor_box)
        #print(x, y, z)
        #print(src.shape())
        if src is None:
        ...
        if i % 1000 == 0:
            np.save("angle-{}.npy".format(record_id), np.array(angles))
            np.save("box-{}.npy".format(record_id), np.array(boxes))
